...
module.exports = {
  ...
  get: async ctx => {
    ...
    var rooms = await mysql("room_impromptu").where('start_date', '>=', today)
    var rooms = await mysql("room_impromptu").innerJoin('cSessionInfo', 'cSessionInfo.open_id', 'room_impromptu.user_id').select('room_impromptu.*', 'cSessionInfo.user_info').orderBy('room_impromptu.create_date', 'desc').limit(limit).offset(offset)
