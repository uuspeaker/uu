...
module.exports = {
  ...
  get: async ctx => {
    ...
    //查询最近10条信息
    if (queryFlag == 0) {
      studyReport = await mysql("user_target").innerJoin('cSessionInfo', 'cSessionInfo.open_id', 'user_target.user_id').select('user_target.*', 'cSessionInfo.user_info').orderBy('user_target.create_date', 'desc').limit(limit).offset(offset)
    }
    //查询前10条信息,此种方式会导致10(limit)条之外的数据无法查询到,因影响不大,使用此种简单但不完备的方式
    if (queryFlag == 1) {
      studyReport = await mysql("user_target").innerJoin('cSessionInfo', 'cSessionInfo.open_id', 'user_target.user_id').select('user_target.*', 'cSessionInfo.user_info').orderBy('user_target.create_date', 'desc').where('user_target.create_date', '>', new Date(firstReportTime)).limit(limit).offset(offset)
    }
    //查询后10条信息
    if (queryFlag == 2) {
      studyReport = await mysql("user_target").innerJoin('cSessionInfo', 'cSessionInfo.open_id', 'user_target.user_id').select('user_target.*', 'cSessionInfo.user_info').orderBy('user_target.create_date', 'desc').where('user_target.create_date', '<', new Date(lastReportTime)).limit(limit).offset(offset)
    }
    ...
    for (var i = 0; i < studyReport.length; i++) {
      ...
      //查询出所有点赞的人，并附到每一条复盘上
      var likeUser = await mysql("user_report_like").innerJoin('cSessionInfo', 'cSessionInfo.open_id', 'user_report_like.user_id').select('user_report_like.user_id', 'cSessionInfo.user_info').where({ 'user_report_like.report_id': studyReport[i].target_id })
      ...
      var commentList = await mysql("user_report_comment").innerJoin('cSessionInfo', 'cSessionInfo.open_id', 'user_report_comment.user_id').select('user_report_comment.user_id', 'cSessionInfo.user_info', 'user_report_comment.comment_id', 'user_report_comment.comment', 'user_report_comment.create_date').where({ 'user_report_comment.report_id': studyReport[i].target_id })
